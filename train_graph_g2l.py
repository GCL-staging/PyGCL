...
def train(model, optimizer, loader: DataLoader, device, param):
    ...
    for data in loader:
        ...
        optimizer.zero_grad()
        z, z1, z2, g1, g2 = model(data.x, edge_index=data.edge_index, batch=data.batch)
        loss = model.loss(z1, g1, z2, g2, batch=data.batch, eps=1.0, **param)
        loss.backward()
        optimizer.step()
        tot_loss += loss.item()
    return tot_loss
...
def main():
    ...
    parser.add_argument('--param_path', type=str, default='params/MVGRL/proteins_jsd.json')
